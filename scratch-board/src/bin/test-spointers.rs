...
mod tests {
    ...
    impl ListRc {
        fn cons_head(&self) -> Option<i32> {
            match self {
                ListRc::ConsRc(head, _) => Some(*head),
                ListRc::NilRc => None,
            }
        }
        ...
        fn cons_tail(&self) -> Option<Rc<ListRc>> {
            match self {
                ListRc::ConsRc(_, tail) => Some(Rc::clone(tail)),
                ListRc::NilRc => None,
            }
        }
    }
    ...
    #[test]
    fn f_test_rc_list() {
        let a = ListRc::ConsRc(2, Rc::new(ListRc::ConsRc(1, Rc::new(ListRc::NilRc))));
        let ra = Rc::new(a);
        let wa = Rc::downgrade(&ra);

        let b = ListRc::ConsRc(10, Rc::clone(&ra));
        let rb = Rc::new(b);

        let c = ListRc::ConsRc(100, Rc::clone(&rb));
        let rc = Rc::new(c);

        let d = ListRc::ConsRc(1000, Rc::clone(&ra));
        let rd = Rc::new(d);
